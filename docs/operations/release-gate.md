...
当前机器没有 Docker、Podman、nerdctl 或 buildah，且三个 GitHub Actions Workflows 尚未
在 GitHub Runner 触发，因此尚不满足正式 Freeze 的硬门禁，也不得创建 `v0.7.0` tag。
...
未运行不能写成通过。`Skipped` 和 `Not Run` 必须保留原因；不得用“预期可通过”“静态已
覆盖”或 Workflow 已创建替代运行证据。
...
任一硬门禁为 `Not Run`、`Skipped` 或 `Failed` 时，状态必须保持 `Freeze Pending`。当前
Docker 动态验证和 GitHub Runner 证据缺失，因此只能保持 Pending。
