"""Offline streaming contract tests for the OpenAI HTTP adapter."""
...
SENSITIVE_API_KEY = "openai-stream-secret-never-render"
SENSITIVE_PROMPT = "OpenAI stream prompt never render"
SENSITIVE_RESPONSE = "OpenAI raw stream error never render"
STREAM_REQUEST_ID = "openai-stream-request-id"
STREAM_MODEL = "openai-stream-response-model"
...
def _settings(**overrides: Any) -> LLMSettings:
    values: dict[str, Any] = {
        ...
        "default_max_tokens": 672,
        "openai_api_key": SENSITIVE_API_KEY,
        "openai_base_url": "https://openai-stream.example.test/v1",
        "openai_default_model": "openai-stream-default-model",
        "_env_file": None,
    }
...
def _request(**overrides: Any) -> LLMRequest:
    values: dict[str, Any] = {
        ...
        "correlation_id": "openai-stream-correlation",
    }
    values.update(overrides)
...
def _success_stream(*, include_usage: bool = True) -> bytes:
    frames = [
        ...
        _event(_chunk_payload(content=" OpenAI")),
...
async def _collect(provider: OpenAIProvider) -> list[LLMStreamChunk]:
...
@pytest.mark.asyncio
async def test_stream_request_contains_only_supported_fields() -> None:
    ...
    request = _request(
        model_name="request-selected-openai-stream-model",
        temperature=0.9,
        max_tokens=345,
    )
    try:
    ...
    assert str(outgoing.url) == (
        "https://openai-stream.example.test/v1/chat/completions"
    )
    ...
    assert payload == {
        ...
        "model": "request-selected-openai-stream-model",
        "temperature": 0.9,
        "max_completion_tokens": 345,
        "stream": True,
        "n": 1,
        "store": False,
...
async def test_stream_defaults_come_from_configuration() -> None:
    ...
    assert captured_payloads[0]["model"] == "openai-stream-default-model"
    assert captured_payloads[0]["temperature"] == 0.4
    assert captured_payloads[0]["max_completion_tokens"] == 672
...
async def test_content_chunks_and_final_metadata_are_normalized() -> None:
    ...
    source = RecordingByteStream(
        [raw_stream[:19], raw_stream[19:79], raw_stream[79:]]
    )
    ...
    assert [chunk.delta for chunk in chunks] == ["Hello", " OpenAI", ""]
    assert all(chunk.provider_name == "openai" for chunk in chunks)
    ...
    assert chunks[2].usage.input_tokens == 19
    assert chunks[2].usage.output_tokens == 6
    assert chunks[2].usage.total_tokens == 25
    assert source.close_calls >= 1


@pytest.mark.asyncio
async def test_empty_content_still_emits_final_chunk_without_usage() -> None:
    data = _event(_chunk_payload(finish_reason="stop")) + _done()
...
async def test_usage_only_event_does_not_emit_text_chunk() -> None:
    ...
    assert len(chunks) == 3
    assert [chunk.delta for chunk in chunks[:-1]] == ["Hello", " OpenAI"]
...
@pytest.mark.asyncio
async def test_response_and_iterator_close_after_parse_error() -> None:
    source = RecordingByteStream(
