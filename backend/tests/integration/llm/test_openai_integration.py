"""Explicitly opted-in, minimal real OpenAI provider checks."""
...
def _settings(environment: Mapping[str, str]) -> LLMSettings:
    return LLMSettings(
        _env_file=None,
        provider="openai",
        default_model=environment["OPENAI_DEFAULT_MODEL"],
        ...
        openai_default_model=environment["OPENAI_DEFAULT_MODEL"],
    )
...
def _request() -> LLMRequest:
    return LLMRequest(
        ...
        temperature=0.0,
        max_tokens=16,
        correlation_id="openai-integration-minimal",
    )
...
@pytest.mark.asyncio
async def test_openai_minimal_non_streaming_call(
    ...
    provider = OpenAIProvider(_settings(environment))
...
@pytest.mark.llm_integration
@pytest.mark.asyncio
async def test_openai_minimal_streaming_call(
    require_llm_provider_environment: ProviderEnvironmentLoader,
) -> None:
    environment = require_llm_provider_environment(
        "OpenAI",
        ...
        async with asyncio.timeout(_OPERATION_TIMEOUT_SECONDS):
            try:
                async for chunk in iterator:
                    assert chunk.provider_name == "openai"
