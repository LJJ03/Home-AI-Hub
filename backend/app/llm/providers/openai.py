"""OpenAI Chat Completions adapter for normalized text generation."""
...
class OpenAIProvider:
    """Adapt OpenAI HTTP payloads to the frozen provider contract."""
        ...
        *,
        ...
        try:
            ...
            credentials_error = ProviderConfigurationError(
                "OpenAI provider credentials are invalid",
    ...
    def default_model(self) -> str:
        """Return the configured OpenAI default model."""

        return self._default_model

    async def generate(self, request: LLMRequest) -> LLMResponse:
        """Generate one normalized response through OpenAI HTTP."""
    ...
    def _raise_configuration_error(component: str) -> NoReturn:
        raise ProviderConfigurationError(
            f"OpenAI provider {component} is required",
            provider_name=_PROVIDER_NAME,
        )
